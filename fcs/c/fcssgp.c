...
static void mdlStart(SimStruct *S) {
	...
	i=1;
	if (mach_sgp[0]>machNo) {
		b=i; i=m_sgp; }
	else {
		while (i<m_sgp)	{
			if ((mach_sgp[i]>machNo) || (i==m_sgp-1)) {
				b=i; i=m_sgp; }
			else i++; } }

	/* Look for nearest y-vector index OVER wanted value */
	i=1;
	if (alt_sgp[0]>altitude) {
		a=i; i=n_sgp; }
	else {
		while (i<n_sgp)	{
			if ((alt_sgp[i]>altitude) || (i==n_sgp-1)) {
				a=i; i=n_sgp; }
			else i++; } }

	/* Perform linear interpolation for 'sgp' */
	sk2 = sg_int(mach_sgp[b-1],mach_sgp[b],alt_sgp[a-1],alt_sgp[a],
		sgp[a-1][b-1],sgp[a-1][b],sgp[a][b-1],sgp[a][b],machNo,altitude);
