double FUNC_NAME_4(struct STRUCTNAME *adt,double x,double y,double z,double zz,int look) { 
				 /* Interpolate in 4-dimensional data table */
				 /* Before using the function, adt->inrow must be set if adt->dimension != 4 */
				 /* The call is then made with lip4(&crmdeb,beta,de,dn,alpha,look) */
				 /* Note that crmdeb is just an example.. */
  #define DEBUG
  int block;
  int row;
  int firstrow;
  int lastrow;
  int column;
  double newfunc,lip3high,lip3low;
  int OK;
  int leftblock;

  column = adt->dimension - 4;     /* Column for x-values */
  if (adt->dimension == 4) { 
    leftblock = 0;
...
/* Check which area we want to move within */
  if (look == 1) { /* Interpolate */
    ...
    if (adt->dimension == 4) { /* Save the position */
      adt->block0 = block;
    } else {
    ...
    if (OK == 1) { /* Interpolate */
      lip3high = FUNC_NAME_3(adt,y,z,zz,1);
      block = block - 1;
      adt->inrow = adt->where[block][column];
      lip3low = FUNC_NAME_3(adt,y,z,zz,1);
      adt->pd[3] = (lip3high - lip3low)/(adt->table[row][column] - adt->table[row-1][column]);
      newfunc = lip3low + (x - adt->table[row-1][column]) * adt->pd[3];
      adt->func_base[3] = newfunc;
      adt->param_base[3] = x;
    } else {
      newfunc = FUNC_NAME_3(adt,y,z,zz,1);
      adt->func_base[3] = newfunc;
      adt->param_base[3] = x;
      adt->pd[3] = 0;
    }
  } /* (look==1) Interpolation ready */ else { /* Extrapolate */
    newfunc = adt->func_base[3] + (x-adt->param_base[3])* adt->pd[3]
        + (y-adt->param_base[2])* adt->pd[2]
        + (z-adt->param_base[1])* adt->pd[1]
        + (zz-adt->param_base[0])* adt->pd[0];
  } /* newfunc value ready */
  return(newfunc);
} /* End of lip4 */
