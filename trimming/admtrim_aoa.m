% ADMIRE r3.4e Trimming start script.
% Written by Torbjorn Noren and  Gunnar Hovmark.
%
% The script will ask for whatever it might need.
global u00;
global x0t;
...
setup_tasks;      % Only necessary if you want to use pitch and/or roll tracking tasks.
r2deg = 180/pi;
...
[x0c,u0c,y0c,dx0c] = admc_sub_aoa(Mach,Alt,x00,u00,Alpha,dMach_err);
...
if (dx0bare(1)) > 0.05
   fprintf('Accelerating: %9.4f m/s^2\n',dx0bare(1));
elseif  (dx0bare(1)) < -0.05
   fprintf('Decelerating: %9.4f m/s^2\n',dx0bare(1));
end
